...
const InfoCard = ({ title, children }) => (
  <div className="pt-6 px-4 md:px-16 rounded-lg">
    <h4 className="text-lg md:text-xl font-semibold text-gray-800 mb-4">
      {title}
    </h4>
    <div className="w-full md:w-[1050px] relative right-0 md:right-4 mb-4 border border-gray-700/70" />
    <p className="text-gray-600 text-base md:text-xl">{children}</p>
    <div className="w-full md:w-[1050px] relative right-0 md:right-4 mt-4 border border-gray-700/70" />
  </div>
);
  ...
  return (
    <section>
      <div className="pt-8 pb-28 bg-[#ffcc01]">
        <h2 className="text-2xl md:text-4xl font-semibold px-4 md:px-12 py-8 text-gray-900">
      ...
      <div className="relative flex flex-col max-w-full md:max-w-[1150px] bottom-24 left-0 md:left-10 rounded-tl-3xl rounded-br-3xl max-h-full md:max-h-[500px] shadow-lg bg-white">
        {/* Tabs Navigation */}
        <div className="mt-8 px-4 md:px-12 pt-5">
          <ul className="flex flex-wrap space-x-4 md:space-x-8 pb-4">
            {["Training", "Tests", "Analysis"].map((tab) => (
              <li
                key={tab}
                onClick={() => handleTabChange(tab)}
                className={`cursor-pointer text-base md:text-xl font-semibold py-2 md:py-4 px-4 md:px-6 rounded-xl bg-neutral-200 ${
        ...
        <div className="tab-content mt-8 mb-32">
          ...
          {/* Tests Tab */}
          {activeTab === "Tests" && (
            <div className="pt-6 px-4 md:px-16 bg-white rounded-lg">
              <h4 className="text-lg md:text-xl font-semibold text-gray-800 mb-4">
          ...
          {/* Analysis Tab */}
          {activeTab === "Analysis" && (
            <div className="pt-6 px-4 md:px-16 rounded-lg bg-white">
              <h4 className="text-lg md:text-xl font-semibold text-gray-800 mb-4">
      ...
      <section className="bg-[#c4171b] text-white px-4 md:px-16 mt-10 flex flex-col lg:flex-row items-center justify-around">
        {/* Image */}
